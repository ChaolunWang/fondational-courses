...
/*This is the driver program of PassServer class, in which
  ...
  exit "x" option. The passwords entered are hided.
...
int main()
{
    ...
    { 
        ...
        else if(option == "a")                                                 // Checking if user selected option a
        {
			...
			cin>>userName;
            cout<<"Enter password: ";
			struct termios systemFlags, localFlags;
            tcgetattr(fileno(stdin), &systemFlags);
            localFlags = systemFlags;
            localFlags.c_lflag &= ~ECHO;
            localFlags.c_lflag |= ECHONL;
            tcsetattr(fileno(stdin), TCSANOW, &localFlags);
            cin>>passWord;
			tcsetattr(fileno(stdin), TCSANOW, &systemFlags);
